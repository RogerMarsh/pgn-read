...
class PgnUpdate___init__(unittest.TestCase):

    def setUp(self):
        self.pgn = parser.PGNUpdate()

    def tearDown(self):
        del self.pgn

    def test___init___PGNUpdate(self):
        p = self.pgn
        self.assertEqual(p.tokens, [])
        self.assertEqual(p.error_tokens, [])
        self.assertEqual(p.collected_game, None)
        ...
        self.assertEqual(p.piecesquaremoves, [])
        self.assertEqual(len(p.__dict__), 21)
        self.assertEqual(set(p.__dict__), {'tokens',
                                           'error_tokens',
                                           'collected_game',
                                           'tags_in_order',
...
class PgnUpdate_set_position_fen(unittest.TestCase):

    def setUp(self):
        self.pgn = parser.PGNUpdate()
        self.pgn_base = parser.PGNUpdate()
...
class PgnUpdate_add_move_to_game(unittest.TestCase):
    ...
    def test__add_move_to_game_02(self):
        ...
        self.assertEqual(len(p.piecesquaremoves), 0)
        p.add_move_to_game()
        self.assertEqual(len(p.__dict__), len(pend.__dict__))
        self.assertEqual(len(p.ravstack), 1)
        self.assertEqual(p.ravstack[-1],
        ...
        self.assertEqual(len(p.piecesquaremoves), 32)
        self.assertEqual(len(p.tokens), 0)
        self.assertEqual(
            p.positions[0].encode('iso-8859-1'),
            b''.join((b'bKQkq-\xff\xff\x00\x00\x00\x00\xff\xff',
                      b'rnbqkbnrppppppppPPPPPPPPRNBQKBNR')))
        ppsm = p.piecesquaremoves
        self.assertEqual(ppsm[0].encode('iso-8859-1'), b'r\x00\x01\x00')
        self.assertEqual(ppsm[1].encode('iso-8859-1'), b'n\x01\x01\x00')
        self.assertEqual(ppsm[2].encode('iso-8859-1'), b'b\x02\x01\x00')
        self.assertEqual(ppsm[3].encode('iso-8859-1'), b'q\x03\x01\x00')
        self.assertEqual(ppsm[4].encode('iso-8859-1'), b'k\x04\x01\x00')
        self.assertEqual(ppsm[5].encode('iso-8859-1'), b'b\x05\x01\x00')
        self.assertEqual(ppsm[6].encode('iso-8859-1'), b'n\x06\x01\x00')
        self.assertEqual(ppsm[7].encode('iso-8859-1'), b'r\x07\x01\x00')
        self.assertEqual(ppsm[8].encode('iso-8859-1'), b'p\x08\x01\x00')
        self.assertEqual(ppsm[9].encode('iso-8859-1'), b'p\x09\x01\x00')
        self.assertEqual(ppsm[10].encode('iso-8859-1'), b'p\x0a\x01\x00')
        self.assertEqual(ppsm[11].encode('iso-8859-1'), b'p\x0b\x01\x00')
        self.assertEqual(ppsm[12].encode('iso-8859-1'), b'p\x0c\x01\x00')
        self.assertEqual(ppsm[13].encode('iso-8859-1'), b'p\x0d\x01\x00')
        self.assertEqual(ppsm[14].encode('iso-8859-1'), b'p\x0e\x01\x00')
        self.assertEqual(ppsm[15].encode('iso-8859-1'), b'p\x0f\x01\x00')
        self.assertEqual(ppsm[16].encode('iso-8859-1'), b'P0\x01\x00')
        self.assertEqual(ppsm[17].encode('iso-8859-1'), b'P1\x01\x00')
        self.assertEqual(ppsm[18].encode('iso-8859-1'), b'P2\x01\x00')
        self.assertEqual(ppsm[19].encode('iso-8859-1'), b'P3\x01\x00')
        self.assertEqual(ppsm[20].encode('iso-8859-1'), b'P4\x01\x00')
        self.assertEqual(ppsm[21].encode('iso-8859-1'), b'P5\x01\x00')
        self.assertEqual(ppsm[22].encode('iso-8859-1'), b'P6\x01\x00')
        self.assertEqual(ppsm[23].encode('iso-8859-1'), b'P7\x01\x00')
        self.assertEqual(ppsm[24].encode('iso-8859-1'), b'R8\x01\x00')
        self.assertEqual(ppsm[25].encode('iso-8859-1'), b'N9\x01\x00')
        self.assertEqual(ppsm[26].encode('iso-8859-1'), b'B:\x01\x00')
        self.assertEqual(ppsm[27].encode('iso-8859-1'), b'Q;\x01\x00')
        self.assertEqual(ppsm[28].encode('iso-8859-1'), b'K<\x01\x00')
        self.assertEqual(ppsm[29].encode('iso-8859-1'), b'B=\x01\x00')
        self.assertEqual(ppsm[30].encode('iso-8859-1'), b'N>\x01\x00')
        self.assertEqual(ppsm[31].encode('iso-8859-1'), b'R?\x01\x00')

class PgnUpdate_collect_game_tokens(unittest.TestCase):
    ...
    def test_collect_game_tokens_02(self):
        p = self.pgn
        next(p.get_games(test_scores))
        self.assertEqual(len(p.collected_game), 6)
        self.assertEqual(len(p.collected_game[0]), 14)
        self.assertEqual(len(p.collected_game[1]), 14)
        self.assertEqual(set([t.group(2) for t in p.collected_game[0]]),
                         set(p.collected_game[1]))
        ...
        self.assertEqual([t.group() for t in p.collected_game[2][-5:]],
                         ['Rd1', 'Rxd1', 'Nxd1', 'Rd8', '1/2-1/2'])
        self.assertEqual(len(p.collected_game[3]), 0)
        self.assertEqual(len(p.collected_game[4]), 76)
        p_psm = len(p.collected_game[5])
        self.assertEqual(p_psm, 1857)

        # Verify sum(<pieces in each position>) == <total piecesquaremoves>
        pm = parser.PGNDisplayMoves()
        next(pm.get_games(test_scores))
        self.assertEqual(len(pm.collected_game[4]), 78)
        psm_count = 0
        for r in pm.moves[1:-1]:
            psm_count += len([b for b in r[-1][0] if b != ''])
        self.assertEqual(psm_count, p_psm)
...
if __name__ == '__main__':
    ...
    runner().run(loader(PgnUpdate___init__))
    runner().run(loader(PgnUpdate_set_position_fen))
    runner().run(loader(PgnUpdate_add_move_to_game))
    runner().run(loader(PgnUpdate_collect_game_tokens))
