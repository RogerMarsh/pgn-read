...
class Game(unittest.TestCase):
    ...
    def test_02___init__(self):
        ...
        ae(
            sorted(i for i in g.__dict__.items()),
            [
                ("_error_list", []),
    ...
    def test_12_append_other_or_disambiguation_pgn(self):
        ...
        g._full_disambiguation_detected = True
        ae(
            g.append_other_or_disambiguation_pgn(
                re.match(constants.DISAMBIGUATE_PGN, "xc3")
            ),
            None,
        )
        ae(g._state, None)
